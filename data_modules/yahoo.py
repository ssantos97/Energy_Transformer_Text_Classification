...
class YahooDataModule(BaseDataModule):
    ...
    def __init__(self, d_params: dict):
        ...
        # hard-coded stuff
        self.path = "yahoo_answers_topics"  # hf_datasets will handle everything
        self.is_multilabel = True
        self.nb_classes = 10  # neg, pos
    ...
    def _collate_fn(self, samples: list, are_samples_batched: bool = False):
        ...
        # stack labels
        labels = collated_samples["topic"]
        if isinstance(labels, list):
            labels = torch.stack(labels, dim=0)

        # keep tokens in raw format
        tokens = collated_samples["question_title"]
    ...
    def setup(self, stage: str = None):
        ...
        modified_dataset = hf_datasets.load_dataset("yahoo_answers_topics")["train"].train_test_split(
            test_size=0.1
        )
        ...
        if self.tokenizer is None:
            ...
            tok_samples = chain(
                self.dataset["train"]["question_title"],
            )
            self.tokenizer = self.tokenizer_cls(tok_samples)

        # map strings to ids
        def _encode(example: dict):
            example["input_ids"] = self.tokenizer.encode(example["question_title"].strip())
            return example
